		...
		<xsl:if test="test-case">
			<testsuite tests="{@testcasecount}" time="{@duration}" errors="{@testcasecount - @passed - @skipped - @failed}" failures="{@failed}" skipped="{@skipped}" timestamp="{@start-time}">
				<xsl:attribute name="name">
					<xsl:for-each select="ancestor-or-self::test-suite[@type='TestSuite' or @type='TestFixture']/@name">
	...
	<xsl:template match="//properties">
		<properties>
			<xsl:for-each select="property">
				<xsl:if test="starts-with(@value, 'TR-')">
					<property name="test_id" value="{substring(@value,4)}"></property>
				</xsl:if>
				<xsl:if test="not(starts-with(@value, 'TR-'))">
